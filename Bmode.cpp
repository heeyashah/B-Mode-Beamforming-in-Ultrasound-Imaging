...
int main()
{
    ...
    BmodeClass *currentScanline = scanlineHead;            // create a new pointer called currentScanline and assign scanlineHead to it
    currentData = dataHead;                                // set currentData to dataHead (points to 1st element in the list)
    for (int i = 1; i < parameters->getNumScanline(); i++) // start iterations at 1 and go until numScanline
    {
        currentData = currentData->next; // set currentData to point to the next data's memory address
        // puts scanline data into a linked list
        currentScanline->next = new BmodeClass(parameters, currentData, i); // creates a new object and assigns it to next
        currentScanline = currentScanline->next;                            // next is now currentScanline
        currentScanline->next = NULL;                                       // original "next" scanline is now NULL (points to the end of the list)
    }
